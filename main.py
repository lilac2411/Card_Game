...
def scoreBoard(surface):
    pygame.draw.rect(surface, BLUE, [590, 0, 210, 600])
...
while not done:
    ...
    if cardTotal(playerHand) > 21:
        ...
        # Whoever's hand total is greater (but still less than 21) wins
    ...
    for index, card in enumerate(dealerHand[1:]):
        card.xPos = 100 + (index + 1) * 80
        card.draw(screen)
    
    # Displaying the player's cards on the table
    for index, card in enumerate(playerHand):
        card.xPos = 100 + index * 80 
        card.draw(screen)
